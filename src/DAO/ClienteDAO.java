...
public class ClienteDAO extends BaseDAO implements DAO<Cliente> {
	...
	public void load() {
		...
		CarrelloDAO carrelloDao=CarrelloDAO.getInstance();
		Cliente client = null;
		CartaFed cf = null;
		RichiestaAcquisto r = null;
		Carrello carrelli = null;
		...
		try {
			...
			while(result.next()) {
				client = new Cliente(result.getString(1), result.getString(2), result.getString(3), result.getString(4), null ,null, null);
				cf = cartaDAO.getCarte(result.getString(5));
				r=richiesta.getRichieste(result.getString(6));
				carrelli=carrelloDao.getCarrello(result.getString(7));
				...
				client.setRichiesta(r);

				client.setCarrello(carrelli);
	...
	public boolean add(Cliente cliente) {
		...
		try {
			...
			preparedStatement.setString(7, cliente.getCarrello().getIdC());
			preparedStatement.execute();
			load();
			return true;
		} catch(SQLException e) {
	...
	public boolean update(Cliente cliente) {
		...
		try {
			...
			ps.setString(1,  cliente.getCF().getCodice());
...
private static final String UPDATE_QUERY_CF = "UPDATE clienti SET id carta fedeltà =(?,?,?,?,?)";
private static final String UPDATE_QUERY_CART = "UPDATE clienti SET id carrello =(?,?,?,?,?,?,?)";
