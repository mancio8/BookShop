...
public class CartaFedDAO extends BaseDAO implements DAO<CartaFed> {
 ...
 public void load() {
	 ...
	 try {
		 ...
		 while(result.next()) {
			 cf = new CartaFed(result.getString(1), result.getDate(2), result.getInt(3), null);
 ...
 public boolean add(CartaFed carta) {
	 ...
	 try {
		 ...
		 ps.setString(4, carta.getCliente().getEmail());
	...
	public boolean update(CartaFed carta) {
		...
		public  CartaFed getCarte(String codice){
			CartaFed carta = null;
 ...
 private static final String UPDATE_QUERY = "UPDATE carte SET datas =(?,?) , punti =(?,?,?) WHERE idC=? ";
}
