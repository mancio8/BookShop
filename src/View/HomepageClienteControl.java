...
public class HomepageClienteControl {
    ...
    public boolean addCarrello(ActionEvent event) {
        ...
        for(Carrello cart : carrelli){
            ...
            if (TFIDc.getText().equalsIgnoreCase(cart.getIdC()) && TextPordotto.getText().equalsIgnoreCase(cart.getProdotto())) {
                ...
                clienti = ClienteDAO.getInstance().getCliente();
                for (Cliente c : clienti) {
                    if (c.getCarrello().equalsIgnoreCase(TFIDc.getText())) {
                        mail = c.getEmail();
                        b = true;
                    }
                }
                ...
                if (b && prod && Integer.parseInt(Tquantita.getText()) > 0 && Integer.parseInt(Tquantita.getText()) <= quant) {
                    ...
                    try {
                        ...
                        ps.setInt(3, Integer.parseInt(Tquantita.getText()));
                        ps.setDouble(4, prezzo * Integer.parseInt(Tquantita.getText()));
                        ps.setString(5, mail);
                        ps.execute();
   ...
   void idC(MouseEvent mouseEvent){
    ...
    private static final String CREATE_QUERY_C ="INSERT INTO carrelli VALUES(?,?,?,?,?)";
    ...
    private static LinkedList<Carrello> carrelli;
    ...
    private static final String UPDATE_QUERY = "UPDATE carrelli SET Quantità=(?), Prezzo=(?) WHERE Prodotti=(?), Email=(?)";
