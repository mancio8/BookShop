...
public class GestioneCcontrol {
        ...
        for ( Utente u : utenti) {
            if (u.getEmail().equals(TFEmailCf.getText())) {
                ...
                for (CartaFed c : carte) {
                    if (c.getCodice().equals(TFid.getText())) {
                        try {
                            AnchorPane root = (AnchorPane) FXMLLoader.load(getClass().getResource("/View/Errore.fxml"));
                            Scene scene = new Scene(root);
                            Stage stage = new Stage();
    ...
    public boolean AddCliente(ActionEvent event) {
        String cartafed=null;
        carte = CartaFedDAO.getInstance().getCarta();
        for (CartaFed cf : carte) {
            if (cf.getCliente().equals(TFEmailCliente.getText())) {
                cartafed = cf.getCodice();
        ...
        for (Utente cli : utenti){
            if(cli.getEmail().equals(TFEmailCliente.getText()) ){
                if (TFEmailCliente.getText().equals("") || TFEmailCliente.getText().length()>100 ) {
                    try {
                        AnchorPane root = (AnchorPane) FXMLLoader.load(getClass().getResource("/View/Errore.fxml"));
                        Scene scene = new Scene(root);
                        Stage stage = new Stage();
                ...
                for (Cliente c : clienti) {
                    if (c.getEmail().equals(TFEmailCliente.getText())) {
                        try {
                            AnchorPane root = (AnchorPane) FXMLLoader.load(getClass().getResource("/View/Errore.fxml"));
                            Scene scene = new Scene(root);
                            Stage stage = new Stage();
                ...
                try {
                    ...
                    preparedStatement.setString(3, TFEmailCliente.getText());
                    preparedStatement.setString(4, password);
                    preparedStatement.setString(5, cartafed);
                    preparedStatement.setString(6, null);
                    preparedStatement.setString(7, TFidC.getText());
